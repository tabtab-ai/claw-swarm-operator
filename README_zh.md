...
> 以下三个参数决定 OpenClaw 实例如何对外暴露，请根据实际集群情况修改：
>
> | 参数 | 说明 |
> |------|------|
> | `operatorConfig.ingressDomain` | OpenClaw 实例 Ingress 规则的基础域名（如 `claw.example.com`） |
> | `operatorConfig.ingressClassName` | 集群 Ingress 控制器对应的 class 名称（如 `kong`、`nginx`） |
> | `operatorConfig.poolSize` | 预热保持的空闲实例数量 |
